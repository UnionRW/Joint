...
import com.corrodinggames.rts.union.gameFramework.j.class_1032 as PacketHess
 ...
 */
class NetEnginePackaging(private val netEngine: class_1001, private val playerConnectX: PlayerConnectX) {
    /**
    ...
    fun addPacket(packet: Packet) {
        ...
        if (packet.type.typeInt > 100) {
            netEngine.method_2737(packetHess)
        } else {
            netEngine.field_5889.add(packetHess)
        }
    }
    ...
    fun transformHessPacket(packet: Packet): PacketHess {
        return PacketHess(packet.type.typeInt).apply {
            field_6125 = packet.bytes
            field_6123 = playerConnectX
        }
    }
    ...
    fun transformPacket(packetHess: PacketHess): Packet {
        return Packet(IRwHps.packetType.from(packetHess.field_6124), packetHess.field_6125)
    }
